...
- **Dead/stale code misleads readers:** ~~`deep_ensemble.py.bak`~~ (deleted 2026-07-04); ~~unused `get_design_matrix`~~ (now the core of the §7.2 linear solver); still open: retracted-caveat comments in `compare_profile_features.py:14` (M8) and large commented-out feature blocks in `pipeline.py:284-430`. Delete — git history keeps them.
- **Fragile idioms worth replacing:** the `sys.modules['pipeline']` pickle hack (`pipeline.py:47-54`) breaks on import-path changes — save scalers/PCA via a stable module path or joblib; blanket `try/except: continue` in Phase 3 (already in §3-Minor) hides failure modes; `_check_file_exists_remote`'s inconsistent return types (§7.1).
- **File size:** `fitting.py` (1939 lines) mixes data loading, physics, fitting, footprint geometry, and HDF5 I/O in one module; `demo_combined.py` (1656) mixes orchestration with per-phase logic. Split along those seams (loader / fit-core / geometry / io) — mostly cut-and-paste, and it makes the methods-section ↔ code mapping obvious for readers of the eventual code release.

...
- **M1 — Target circularity + selection effect.** Label = OCO-2 minus OCO-2 clear-sky neighbors (both `xco2_bc`); no independent anchor. Real sub-0.25° CO2 gradients are labeled "cloud anomaly." `std_thres` drops the hardest (heavily cloudy) scenes from the label set. Needs: negative control (correction ≈ 0 far from clouds; no degradation over TCCON on clear days) and a check against a transport-model field.
...
- **M3 — No significance testing.** 43/70 station-days improved in |bias| is near chance; days clustered by site (Réunion 14, Ny-Ålesund 9, Wollongong 7) violate independence. Five sites worsen in mean bias after correction (et, js, ka, or, rj). Needs paired Wilcoxon + site-clustered bootstrap CIs; per-site pooling for headline.
...
- **M6 — `xco2_raw_minus_apriori` leakage risk.** Nearly collinear with `xco2_bc`, half the target definition. Report headline TCCON results for `no_xco2` alongside `full`.
...
- Temporal buffer threshold: 2022 (`workspace/demo_combined.py:738`) vs 2023 (`phase_03_processing.py:1540` docstring/default) — reconcile.
- Target-param doc drift: production 0.25°/1.0 ppm/10 km vs 0.5/2.0/10 in `build_feature_dataset.py:52` and `pipeline.py:736`.
- CLI loss default `gaussian_nll` vs production `beta_nll` (`deep_ensemble.py:331`).
- Band-width/overlap defaults differ across three call sites (2.5/0.5 vs 10/1 vs 5/1); `process_orbit` default order (7,2,7) vs used (7,3,7).
- Phase-3 blanket `try/except: continue` hides per-sounding failure accounting.
- Train-date exclusion from TCCON eval is manual comment-based — replace with programmatic check against a training-date manifest.

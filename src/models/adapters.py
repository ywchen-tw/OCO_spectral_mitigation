...
class FTAdapter(ModelAdapter):
    ...
    def load(cls, output_dir, device: torch.device | None = None) -> 'FTAdapter':
        ...
        if _main is not None and hasattr(_main, 'UncertainFTTransformerRefined'):
            ...
            from .transformer import UncertainFTTransformerRefined
...
class TabMAdapter(ModelAdapter):
    ...
    Follows the FTAdapter pattern: model_tabm_best.pt is written by the training
...
class HybridAdapter(ModelAdapter):
    ...
    def load(cls, output_dir, device: torch.device | None = None) -> 'HybridAdapter':
        ...
        from .hybrid import HybridDualTower
...
class FTClassifierAdapter(ModelAdapter):
    ...
    def load(cls, output_dir, device: 'torch.device | None' = None) -> 'FTClassifierAdapter':
        ...
        if _main is not None and hasattr(_main, 'FTClassifier'):
            ...
            from .classifier_transformer import FTClassifier

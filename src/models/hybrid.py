...
def plot_hybrid_evaluation_by_regime(model: HybridDualTower,
    ...
    """
    from .transformer import plot_evaluation_by_regime
...
def main():
    ...
    if n_bad > 0:
        ...
        print(f"  WARNING: dropping {n_bad} rows with NaN/Inf features ({finite_mask.sum()} remain)", flush=True)
        X = X[finite_mask]
        y = y[finite_mask]
    ...
    if _pc1_col is not None:
        from .transformer import _batched_predict as _bp_eval

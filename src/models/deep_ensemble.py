...
def main():
    ...
    p.add_argument('--member_archs', type=str, default=None,
                   help="HETEROGENEOUS deep-ensemble (DE++): semicolon-separated list "
                        "of per-member architectures, each a comma-separated width "
                        "spec, e.g. '64,32;128,64;256,128,64;96,48'. Member m uses "
                        "archs[m %% len(archs)] (cycled), so members differ in "
                        "width/depth -> more decorrelation than the homogeneous "
    ...
    # members differ structurally (more decorrelation).  Unset -> all use hidden_dims.
    if args.member_archs:
        arch_pool = [tuple(int(x) for x in a.split(',') if x.strip())
                     for a in args.member_archs.split(';') if a.strip()]
        member_dims = [arch_pool[m % len(arch_pool)] for m in range(args.n_members)]
        print(f"[deep_ensemble] DE++ heterogeneous archs (pool of {len(arch_pool)}): "
              + ", ".join(str(d) for d in member_dims))
    else:
        member_dims = [hidden_dims] * args.n_members
    members = []

...
def fit_spectral_model_test(tau, ln_T, fit_order):
    """Fit a cumulant expansion to smoothed log-transmittance vs optical depth.
    ...
    tau_sorted  = tau[sort_idx]
    ln_T_sorted = ln_T[sort_idx]
    # Adapt the Savitzky-Golay window to available valid channels.
    # A fixed window_length=51 fails for short masked spectra.
    n_pts = ln_T_sorted.size
    if n_pts >= 5:
        window_length = min(51, n_pts if n_pts % 2 == 1 else n_pts - 1)
        polyorder = min(3, window_length - 1)
        ln_T_smooth = savgol_filter(ln_T_sorted, window_length=window_length, polyorder=polyorder)
    else:
    ...
    # Intercept corresponds to ln(T) at tau=0. Constrain it using robust
    # high-quantile ln(T) plus a small margin, and low-tau channels for scale.
    # This is stricter than an unconstrained fit but less brittle than
    # intercept_ub = max(ln_T), which can fail with noisy single-point maxima.
    eps = 1e-6
    tau_p10 = np.nanpercentile(tau_sorted, 10)
    low_tau_mask = tau_sorted <= tau_p10

    q_hi = float(np.nanpercentile(ln_T_sorted, 99.5))
    if np.any(low_tau_mask):
        q_lo_tau = float(np.nanpercentile(ln_T_sorted[low_tau_mask], 80))
    else:
        q_lo_tau = float(np.nanpercentile(ln_T_sorted, 80))

    intercept_ub = q_hi + 0.02
    intercept_lb = q_lo_tau - 0.5
    if not np.isfinite(intercept_lb):
        intercept_lb = -np.inf
    if not np.isfinite(intercept_ub):
        intercept_ub = np.inf
    if np.isfinite(intercept_lb) and np.isfinite(intercept_ub) and intercept_lb >= intercept_ub:
        intercept_lb = intercept_ub - 0.1

    lb[-1] = intercept_lb
    ub[-1] = intercept_ub

    # Trim extreme optical depths for fitting stability (especially O2A tails).
    # The model is then still evaluated on full range later.
    tau_trim_hi = np.nanpercentile(tau_sorted, 99.5)
    fit_mask = tau_sorted <= tau_trim_hi
    min_fit_points = max(25, n_params + 2)
    if np.count_nonzero(fit_mask) >= min_fit_points:
        tau_fit_data = tau_sorted[fit_mask]
        ln_fit_data = ln_T_smooth[fit_mask]
    else:
        tau_fit_data = tau_sorted
        ln_fit_data = ln_T_smooth

    # Fit in a scaled-tau space to reduce high-order numerical instability at
    # large optical depth (especially O2A tails), then convert coefficients
    # back to the original tau basis.
    tau_scale = float(np.nanpercentile(tau_fit_data, 90))
    if (not np.isfinite(tau_scale)) or (tau_scale <= 0):
        tau_scale = 1.0
    tau_fit_scaled = tau_fit_data / tau_scale

    # Weight small-tau channels more heavily so the fit is anchored near tau=0,
    # where intercept/k1 are most physically informative.
    tau_min = float(np.nanmin(tau_fit_scaled))
    tau_max = float(np.nanmax(tau_fit_scaled))
    tau_span = max(tau_max - tau_min, eps)
    tau_norm = (tau_fit_scaled - tau_min) / tau_span  # 0 at small tau, 1 at large tau
    weights = 1.0 / (1.0 + 9.0 * tau_norm**2)         # in [~0.1, 1.0]
    sqrt_w = np.sqrt(weights)

    # Build a bounded linear least-squares prefit to get a stable p0.
    # beta = [intercept, k1, k2, ..., kN] under the same sign convention as get_design_matrix.
    X = get_design_matrix(tau_fit_scaled, fit_order)
    lb_beta = np.full(n_params, -np.inf)
    ub_beta = np.full(n_params, np.inf)
    lb_beta[0] = intercept_lb
    ub_beta[0] = intercept_ub
    if fit_order >= 1:
        lb_beta[1] = 0.0
    if fit_order >= 2:
        lb_beta[2] = 0.0

    # Weighted linear prefit: minimize ||sqrt(w) * (X beta - y)||^2
    Xw = X * sqrt_w[:, None]
    yw = ln_fit_data * sqrt_w
    res = lsq_linear(Xw, yw, bounds=(lb_beta, ub_beta), method="trf")
    if not res.success:
        raise ValueError(f"lsq_linear failed (status={res.status})")

    beta0 = res.x
    # Convert to curve_fit parameter order in scaled-tau basis:
    # [k1, k2, ..., kN, intercept]
    p0 = np.concatenate([beta0[1:], [beta0[0]]])
    def model_scaled(tau_raw, *params):
        return model_func(tau_raw / tau_scale, *params)

    popt_scaled, _ = curve_fit(
        model_scaled,
        tau_fit_data,
        ln_fit_data,
        sigma=1.0 / np.maximum(sqrt_w, eps),  # curve_fit uses 1/sigma^2 as weights
        absolute_sigma=False,
        bounds=(lb, ub),
        p0=p0,
    )

    # Convert fitted parameters back to original tau basis so downstream
    # diagnostics/plots keep the same interpretation.
    popt = np.array(popt_scaled, dtype=float)
    for i in range(1, fit_order + 1):
        popt[i - 1] = popt_scaled[i - 1] / (tau_scale ** i)
...
def plot_fitting_example(tag, fp, sounding_ind, wvl, rad, transmittance, tau, ln_T, popt_log, fit_order, output_dir):
    ...
    with plt.rc_context(_rc):
        ...
        ax1.legend(l1 + l2, [l.get_label() for l in l1 + l2],
                   loc="lower left", bbox_to_anchor=(0.14, 1.02),
...
def process_orbit(sat, orbit_id, shared_data, fit_order=(7, 2, 7), overwrite=True):
    ...
    if not os.path.isfile(output_file) or overwrite:
        ...
        fit_stats = {
            tag: {"too_few_points": 0, "fit_error": 0, "success": 0}
            for tag in tags
        }
        ...
        for tag in tags:
            st = fit_stats[tag]
            logger.info(
                f"[{orbit_id}] {tag}: success={st['success']} "
                f"too_few_points={st['too_few_points']} fit_error={st['fit_error']}"
            )
            if st["success"] == 0:
                logger.warning(
                    f"[{orbit_id}] No successful {tag} fits; no {tag}_log_T_fit_*.png generated."
                )
...
def main():
    ...
    parser.add_argument('--use-ring', action='store_true',
                        help='Apply Ring effect (rotational Raman) correction to O2A optical depth')
...
python src/spectral/fitting_with_ring_effect.py \
        --date 2018-10-18 --limit-granules 1 --visualize \
        --data-dir /Users/yuch8913/programming/oco_fp_analysis/data \
        --output-dir /Users/yuch8913/programming/oco_fp_analysis/results_with_ring_effect \

...
from abs_util.create_atm import create_oco_atm
...
class cld_sat:
    ...
    def __init__(self, \
        ...
        if ((self.fname is not None) and (os.path.exists(self.fname)) and (not overwrite)):
            ...
            self.run(cth, cgt, dz, zpt_file, fname_atm, sat_info)
            self.dump(self.fname)

        elif ((self.sat is not None) and (self.fname is None)):

            self.run(self.sat, cth, cgt, dz, zpt_file, fname_atm)
    ...
    def run(self, cth, cgt, dz, zpt_file, fname_atm, sat_info):
        ...
        # process
        self.process(self.sat, zpt_file, fname_atm, sat_info,
                     cloud_top_height=cth, cloud_geometrical_thickness=cgt, layer_thickness=dz)
    ...
    def process(self, sat_obj, zpt_file, fname_atm, sat_info,
                cloud_geometrical_thickness=1.0, cloud_top_height=3.0, layer_thickness=1.0):
        ...
        cloud_top_height[cloud_top_height==0] = np.nan
        ...
        print('levels:', levels)
        cld_zpt_file = zpt_file.replace('zpt.h5', 'cld_zpt.h5')
        if not os.path.isfile(cld_zpt_file):
            create_oco_atm(sat=sat_info, o2mix=0.20935, output=cld_zpt_file, new_h_edge=levels)

        self.atm = atm_atmmod(zpt_file=cld_zpt_file, fname=fname_atm, overwrite=True)

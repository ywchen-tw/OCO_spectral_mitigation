...
def oco_fp_atm_abs(sat=None, o2mix=0.20935, output='fp_tau_combined.h5',
                   oco_files_dict=None,
                    oco_nc_file=None,
                   overwrite=False, use_ring=False):
    """

    """
    
    abs_skip = _tau_file_is_current(output, use_ring=use_ring) and not overwrite
    ...
    if sat == None:
        ...
        ring_o2a_output_all = np.zeros((final_length, 1016))
        ...
        for i in range(0, final_length, processing_length):
            ...
            need = (overwrite or not _tau_file_is_current(output_tmp, use_ring=use_ring)) and not abs_skip
            if need:
                ...
                (tau_o2a,  me_o2a,  sol_o2a, ring_o2a), \

...
#SBATCH --job-name=oco_train_de_foldpca_r05
...
# ── Production DE (ocean, 5 km target) retrained with FOLD-SPECIFIC ProfilePCA ─
# Identical config to the production run in curc_shell_blanca_de_profile_r05.sh
...
#   results/profile_pca/profile_pca_ocean.pkl.
#
...
#   de_ocean_beta_nll_prof_reg_r05_f${F}          (global PCA, production)
#   de_ocean_beta_nll_prof_reg_foldpca_r05_f${F}  (this script)
#
# PREREQUISITES:
...
#     --dirs 'results/model_deep_ensemble/de_ocean_beta_nll_prof_reg_foldpca_r05_f*' \
#     --label DeepEns+foldpca+r05 \
#     --out results/model_comparison/deep_ensemble_ocean_profile_foldpca_r05_kfold_agg.md
...
python -m models.deep_ensemble --sfc_type 0 --suffix de_ocean_beta_nll_prof_reg_foldpca_r05_f${F} \
...
# ── Feature-set ablations (+fold-specific profile, 5 km reference) ─────────────
# Same config as the production run above, each with one feature block dropped,
# ocean only.  The profile block is ORTHOGONAL to --feature_set; each ablation

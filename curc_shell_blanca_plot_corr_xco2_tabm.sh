...
#SBATCH --job-name=oco_plot_tabm
#SBATCH --account=blanca-airs
###SBATCH --partition=blanca-airs
#SBATCH --qos=preemptable

# TabM version of curc_shell_blanca_plot_corr_xco2_deepens.sh.
# For each case it (4) applies the per-surface TabM ensemble on ocean AND land
# footprints (build_tabm_plot_data.py) then (5) plots vs TCCON + MODIS
# (plot_corrected_xco2.py, --poster-model tabm_corrected_xco2).
#
...
SCRIPT_NAME=curc_shell_blanca_plot_corr_xco2_tabm.sh
...
# ─── model (TabM fold dirs; per-surface) ──────────────────────────────────────
# Pool ALL date_kfold folds (f0..f4) → cross-fold ensemble: the point prediction
# is the mean over folds of TabM's mean-member q50 (each fold transformed by its
# OWN fitted pipeline / scaler), mirroring the cross-fold pooling used for the DE.
# Each TabM already carries the vertical-profile EOF block (the `_prof` tag: 12 EOF
# PCs of the sigma-grid T/q/CO2-prior profiles + 2 tropopause scalars, appended to
# the `full` feature set — embedded in each fold's pipeline pkl, so no extra build
# flag is needed).  Trained by curc_shell_blanca_train_tabm_r05.sh / _r15.sh.
# Snow: both surfaces KEEP snow footprints (CLI default; --exclude_snow would drop
# them), and snow_flag is NOT a feature — so high-latitude/snow land is corrected
# in-domain, not flagged OOD.
...
# profile-PCA transform in build_tabm_plot_data.py raises KeyError.
# MODEL_TAG names the model version; it namespaces OUT_BASE so different model
...
# variant, LAND uses r15 (1.5°) — each surface's best-performing profile model
# (from curc_shell_blanca_train_tabm_r05.sh / _r15.sh).  Tag reflects the mix.
# NOTE: build_tabm_plot_data.py has no cloud-classifier hook, so (unlike the DE
# script) there are no per-surface xgb_cloud dirs here — only tabm_corrected_xco2.
MODEL_TAG=tabm_prof_o05l15
OCEAN_MODEL_DIRS=("$DATA_ROOT"/results/model_tabm/tabm_ocean_prof_r05_f*)
LAND_MODEL_DIRS=("$DATA_ROOT"/results/model_tabm/tabm_land_prof_r15_f*)

CSV_DIR="$DATA_ROOT"/results/csv_collection
# OUT_BASE is namespaced by MODEL_TAG so each model version writes to its own tree
# (e.g. .../tabm/tabm_prof_o05l15/combined_<date>_<site>/).
OUT_BASE="$DATA_ROOT"/results/model_comparison/tabm/${MODEL_TAG}
...
run_case() {
    ...
    # (4) apply TabM ensemble → plot_data.parquet
    # REBUILD=0 (default) reuses an existing plot_data.parquet that is newer than its
    # input parquet, so a re-run only fills in missing cases.  REBUILD=1 forces a rebuild.
    if [[ "${REBUILD:-0}" != 1 && -f "$plotdata" && "$plotdata" -nt "$input" ]]; then
        ...
        python workspace/build_tabm_plot_data.py \
            "${model_args[@]}" --input "$input" --output "$plotdata" || { echo "  build failed"; return; }
    fi

    # (5)+(6) per-case MODIS-overlap figures (plot_corrected_xco2 + plot_spectral_params).
    # These are the expensive --modis-auto maps.  MAKE_PLOTS=0 skips them so a run can
    # produce ONLY the plot_data.parquet needed by the aggregate tccon_comparison_report.
    if [[ "$MAKE_PLOTS" != 0 ]]; then
        # (5) plot
        local h5_arg=(); [[ -f "$h5" ]] && h5_arg=(--results-h5 "$h5")
        local poster_arg=(); [[ "$poster" == poster ]] && \
            poster_arg=(--poster-model tabm_corrected_xco2 --poster-dpi 300)

...
def make_fig(date, radius_km, twin_s, do_modis):
    ...
    a = ax[1, 0]; _bg(a)
    a.scatter(inx.lon, inx.lat, c=inx.xco2_bc, s=10, cmap=CMAPS["xco2"], vmin=vmin, vmax=vmax,
              alpha=0.5, zorder=2, edgecolors="none")
    sc = a.scatter(pooled.lon, pooled.lat, c=pooled.xco2_bc, s=16, cmap=CMAPS["xco2"],
    ...
    a.set(xlim=(lon0, lon1), ylim=(lat0, lat1), xlabel="Lon (°E)", ylabel="Lat (°N)",
          title=f"Original bias-corrected {XCO2_LABEL} (before correction)" + (" (MODIS Aqua)" if bg is not None else ""))
    a.set_aspect(asp)
    ...
    for i, L in enumerate(legs):
        ...
        a.plot(cb, pb, "-", color=col, lw=1.5, label=f"ATom leg {L['pid']} (in-situ CO2)")

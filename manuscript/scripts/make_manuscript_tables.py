...
Tables written to manuscript/tables/:
  ...
  tabC8_raw_bc_ml.tex        raw / bc / ML-on-bc / ML-on-raw four series (AK)
...
# Cloud-distance slices use the PRODUCTION per-surface target radii
# (ocean 5 km / land 15 km, Sect. 3.1) since 2026-07-23: each surface's
# near/far split is read from the report edition run with the matching
# --cld-edges (suffixes _cldo5 / _cldl15; pooled rows identical in both).
SUFFIXES = {"o5": "_cldo5_r100km", "l15": "_cldl15_r100km"}
SLICES = [
    ("Pooled, QF 0+1", "all", "all", "all", "o5"),
    ("Pooled, QF$\\,$=$\\,$0", "qf0", "all", "all", "o5"),
    ("Pooled, QF$\\,$=$\\,$1", "qf1", "all", "all", "o5"),
    ("Ocean", "all", "ocean", "all", "o5"),
    ("Ocean, QF$\\,$=$\\,$1", "qf1", "ocean", "all", "o5"),
    ("Ocean $\\leq$5\\,km", "all", "ocean", "0–5 km", "o5"),
    ("Ocean $\\leq$5\\,km, QF$\\,$=$\\,$1", "qf1", "ocean", "0–5 km", "o5"),
    ("Ocean $\\geq$5\\,km", "all", "ocean", "≥5 km", "o5"),
    ("Land", "all", "land", "all", "l15"),
    ("Land, QF$\\,$=$\\,$1", "qf1", "land", "all", "l15"),
    ("Land $\\leq$15\\,km", "all", "land", "0–15 km", "l15"),
    ("Land $\\leq$15\\,km, QF$\\,$=$\\,$1", "qf1", "land", "0–15 km", "l15"),
    ("Land $\\geq$15\\,km", "all", "land", "≥15 km", "l15"),
]


def load_metrics(tree):
    """{edition: metrics dataframe} for one model tree."""
    return {ed: pd.read_csv(tree / f"tccon_metrics_ak{sfx}.csv")
            for ed, sfx in SUFFIXES.items()}
...
def table_model_comparison():
    ...
    rows = []
    for label, qf, surface, cld, ed in SLICES:
        rr = {m: srow(mets[m][ed], qf, surface, cld) for m in TREES}
    ...
    cap = ("Footprint RMSE against AK-harmonized TCCON (ppm; 100\\,km / "
           ...
           "cloud-distance slices split each surface at its production "
           "target radius (ocean 5\\,km, land 15\\,km; Sect.~3.1) and use "
           "the pre-drift cases where the Aqua-MODIS collocation is "
           "reliable.")
...
def table_ablation():
    ...
    cap = ("Feature-group ablation of the mix deep ensemble on TCCON "
           ...
           "Cloud-distance slices split each surface at its production "
           "target radius (ocean 5\\,km, land 15\\,km; Sect.~3.1). "
...
def table_raw_bc_ml():
    a = load_metrics(TREES["DE"])
    b = load_metrics(RAW_TREE)
    rows = []
    for label, qf, surface, cld, ed in SLICES:
        ra, rb = srow(a[ed], qf, surface, cld), srow(b[ed], qf, surface, cld)
...
def main():
    ...
    for name, fn in [("tab_model_comparison", table_model_comparison),
                     ...
                     ("tabC8_raw_bc_ml", table_raw_bc_ml),
